...
#ifndef LINT
static char *vcid = "$Id: connect.c,v 1.85 2004/09/06 11:31:44 gotoh Exp $";
#endif
...
*/
static char *usage = "usage: %s [-dnhs45] "
#ifdef _WIN32
#ifdef __CYGWIN32__
"[-w timeout] \n"				/* cygwin cannot -R */
...
open_connection( const char *host, u_short port )
{
    ...
    assert (s < sizeof(fd_set)*8);		/* ensure fd_set is enough */
    return s;
}
...
accept_connection (u_short port)
{
    ...
    assert (connection < 8 * sizeof (fd_set));
    return connection;
}
...
main( int argc, char **argv )
{
    ...
    make_revstr();
    getarg( argc, argv );
    debug("Program is $Revision: 1.85 $\n");
